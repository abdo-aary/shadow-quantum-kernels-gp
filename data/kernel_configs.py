...
KernelType = Literal["rbf", "matern32", "nonstat_amp", "nonstat_ls", "nonstat_sm"]
...
def _kernel_matrix(X: np.ndarray, cfg: KernelConfig) -> np.ndarray:
    ...
    if ktype == "rbf":
        ...
        if d > 1:
            ...
            K = K_1d
    elif ktype == "nonstat_sm":
        # Non-stationary spectral mixture kernel in 1D with input-dependent amplitude
        #
        # Expected params:
        #   - Q: int, number of spectral components
        #   - weights: list/array of length Q (mixing weights)
        #   - means: list/array of length Q (frequencies)
        #   - scales: list/array of length Q (spectral std devs)
        #   - a, b, c: amplitude parameters, as in nonstat_amp
        #
        # We work along the first input dimension and modulate a stationary
        # SM base kernel with an input-dependent amplitude s(t).

        Q = int(p.get("Q", 3))

        # Turn weights/means/scales into numpy arrays with reasonable defaults
        weights = np.asarray(
            p.get("weights", np.ones(Q) / Q),
            dtype=float,
        ).reshape(Q)

        means = np.asarray(
            p.get("means", np.linspace(0.5, 2.5, Q)),
            dtype=float,
        ).reshape(Q)

        scales = np.asarray(
            p.get("scales", 0.3 * np.ones(Q)),
            dtype=float,
        ).reshape(Q)

        if not (
            weights.shape[0] == means.shape[0] == scales.shape[0] == Q
        ):
            raise ValueError(
                f"nonstat_sm expects weights/means/scales of length Q={Q}; "
                f"got shapes {weights.shape}, {means.shape}, {scales.shape}."
            )

        # 1D coordinate we use for the SM kernel
        t = X[:, 0:1]           # (N,1)
        t1 = t
        t2 = t.T
        tau = t1 - t2           # (N,N)
        ...
        two_pi = 2.0 * np.pi
        K_sm = np.zeros_like(D2_t, dtype=float)

        # Stationary 1D spectral mixture base: sum_q w_q^2 exp(-2π^2 v_q τ^2) cos(2π μ_q τ)
        for q in range(Q):
            wq = float(weights[q])
            muq = float(means[q])
            vq = float(scales[q]) ** 2

            K_sm += (wq ** 2) * np.exp(-2.0 * (np.pi ** 2) * vq * D2_t) * np.cos(
                two_pi * muq * tau
            )

        # Input-dependent amplitude s(t) as in nonstat_amp
        a = float(p.get("a", 0.3))
        b = float(p.get("b", 0.6))
        c = float(p.get("c", 2.0))
        ...
        if a < 0.0 or b < 0.0 or a + b > 1.0:
            raise ValueError(
                f"nonstat_sm parameters must satisfy a >= 0, b >= 0, a + b <= 1; got a={a}, b={b}."
            )

        s = a + b / (1.0 + np.exp(-c * t))  # (N,1)
        amp = s @ s.T                       # (N,N)
