...
class MKLearner:
    """
    Multiple Kernel Learner on top of block-local quantum Gram matrices.
    ...
    _Phi_components_train: Optional[np.ndarray] = field(init=False, default=None, repr=False)  # (E, M_tr, r)
    _hutchinson_Z: Optional[np.ndarray] = field(init=False, default=None, repr=False)  # (M_tr, S)
    _KEZ: Optional[np.ndarray] = field(init=False, default=None, repr=False)  # (E, M_tr, S)
    ...
    def __post_init__(self) -> None:
        ...
        # Prepare approximation structures for approximate gradients, if requested
    ...
    def fit(
        ...
        verbose : bool
            If True, prints progress for each GP. In parallel mode
            logs are collected per-GP and printed in contiguous blocks
            to avoid interleaving across GPs.
        n_jobs : int or None
            Number of worker threads to use across GPs.
        ...
        with ThreadPoolExecutor(max_workers=n_jobs) as executor:
            # executor.map preserves order of `tasks`, so GPs are processed
            # in index order and logs are printed in contiguous blocks.
            for g, w_opt, L_opt, alpha_opt, L_chol_opt, logs in executor.map(
                ...
                # print logs in the main thread, grouped by GP
                if verbose:
    ...
    def infer(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for g in range(self.num_gps):
            ...
            # Add jitter to the training kernel (same as in training)
            K_y = K_train + self._noise_plus_jitter * np.eye(K_train.shape[0])

            # Re-factorize in case fit() was not saved; but we actually have L, alpha
            L = self._L_factors[g]
            alpha = self._alpha[g]
            ...
            # predictive variance: diag(K_ss - k_* K^{-1} k_*^T)
            # v = L^{-1} k_*^T
            v = np.linalg.solve(L, K_test_train.T)  # (M_tr, M_te)
            var = np.diag(K_test_test) - np.sum(v * v, axis=0)

            means[g] = mu
            # ensure non-negative variances up to numerical noise
            variances[g] = np.maximum(var, 0.0)
    ...
    def get_train_test_metrics(self) -> Dict[str, np.ndarray]:
        ...
        for g in range(num_gps):
            y_test = y_all[g, te]
            mu = means[g]
            var = vars_[g] + self._noise_plus_jitter  # predictive variance + small noise
    ...
    def load(path: str) -> "MKLearner":
        ...
        with open(path, "rb") as f:
            obj = pickle.load(f)
        if not isinstance(obj, MKLearner):
